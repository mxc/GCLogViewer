...
DataStore.prototype.getObjectArray = function (cursorProvider, callback) {
    var results = [];
    ...
    if (typeof cursorProvider === 'string') {
        cursorProvider = this.dbCon.getObjectStoreFromTransaction(cursorProvider, "readonly",
                function (event) {
                    console.log("worked");
                }, onError);
    }
...
DataStore.prototype.getObjects = function (cursorProvider, callback) {
    var request = cursorProvider.openCursor();
...
function createIndexDBConnection(dbName, version, schema) {
    var db = null;
    var schema = schema;
    var DBConnection = {
        dbName: dbName,
